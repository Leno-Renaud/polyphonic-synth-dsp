...
struct mydsp : public dsp {
	...
	virtual void instanceConstants(int sample_rate) {
		...
		fConst1 = 1.0f / std::max<float>(1.0f, 0.2f * fConst0);
		fConst2 = std::max<float>(1.0f, 0.005f * fConst0);
		fConst3 = 1.0f / fConst2;
		fConst4 = 0.7f / std::max<float>(1.0f, 0.08f * fConst0);
		...
		fConst13 = 3.0f / fConst0;
		fConst14 = 2.0f / fConst0;
		fConst15 = 1.0f / fConst0;
		fConst16 = std::tan(157.07964f / fConst0);
		fConst17 = 1.0f / (fConst16 * fConst9);
	...
	virtual void buildUserInterface(UI* ui_interface) {
		...
		ui_interface->addHorizontalSlider("volume", &fHslider1, FAUSTFLOAT(0.5f), FAUSTFLOAT(0.0f), FAUSTFLOAT(1.0f), FAUSTFLOAT(0.001f));
		ui_interface->closeBox();
	}
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		float fSlow5 = fConst14 * fSlow2;
		float fSlow6 = fConst15 * fSlow2;
		...
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			...
			float fTemp2 = ((iTemp0) ? 0.0f : fConst12 + fRec7[1]);
			fRec7[0] = fTemp2 - std::floor(fTemp2);
			float fTemp3 = 0.003f * ftbl0mydspSIG0[std::max<int>(0, std::min<int>(static_cast<int>(65536.0f * fRec7[0]), 65535))] + 1.0f;
			float fTemp4 = ((iTemp0) ? 0.0f : fRec6[1] + fSlow4 * fTemp3);
			fRec6[0] = fTemp4 - std::floor(fTemp4);
			float fTemp5 = ((iTemp0) ? 0.0f : fRec8[1] + fSlow5 * fTemp3);
			fRec8[0] = fTemp5 - std::floor(fTemp5);
			float fTemp6 = ((iTemp0) ? 0.0f : fRec9[1] + fSlow6 * fTemp3);
			fRec9[0] = fTemp6 - std::floor(fTemp6);
			fRec3[0] = 0.6f * ftbl0mydspSIG0[std::max<int>(0, std::min<int>(static_cast<int>(65536.0f * fRec9[0]), 65535))] + 0.3f * ftbl0mydspSIG0[std::max<int>(0, std::min<int>(static_cast<int>(65536.0f * fRec8[0]), 65535))] + 0.15f * ftbl0mydspSIG0[std::max<int>(0, std::min<int>(static_cast<int>(65536.0f * fRec6[0]), 65535))] + 0.2f * ftbl0mydspSIG0[std::max<int>(0, std::min<int>(static_cast<int>(65536.0f * fRec5[0]), 65535))] - fConst10 * (fConst8 * fRec3[2] + fConst6 * fRec3[1]);
			float fTemp7 = fRec3[2] + fRec3[0] + 2.0f * fRec3[1];
			fVec3[0] = fTemp7;
			fRec2[0] = -(fConst20 * (fConst19 * fRec2[1] - fConst17 * (fTemp7 - fVec3[1])));
			float fTemp8 = fSlow7 * fRec2[0] * std::max<float>(0.0f, std::min<float>(fConst3 * fRec1[0], std::max<float>(fConst4 * (fConst2 - fRec1[0]) + 1.0f, 0.3f)) * (1.0f - fConst1 * static_cast<float>(iRec0[0])));
			output0[i0] = static_cast<FAUSTFLOAT>(fTemp8);
			output1[i0] = static_cast<FAUSTFLOAT>(fTemp8);
			iVec0[1] = iVec0[0];
			fVec1[1] = fVec1[0];
			iRec0[1] = iRec0[0];
			fRec1[1] = fRec1[0];
	...
	#define FAUST_FILE_NAME "test.dsp"
	...
	FAUST_ADDHORIZONTALSLIDER("volume", fHslider1, 0.5f, 0.0f, 1.0f, 0.001f);
	...
	#define FAUST_LIST_ACTIVES(p) \
		...
		p(HORIZONTALSLIDER, volume, "volume", fHslider1, 0.5f, 0.0f, 1.0f, 0.001f) \
...
test::test() : AudioStream(FAUST_INPUTS, new audio_block_t*[FAUST_INPUTS])
{
#ifdef NVOICES
    int nvoices = NVOICES;
...
void test::update(void) { updateImp<FAUST_INPUTS, FAUST_OUTPUTS>(); }

void test::setParamValue(const std::string& path, float value)
{
    fUI->setParamValue(path, value);
}

float test::getParamValue(const std::string& path)
{
